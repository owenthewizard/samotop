...
impl fmt::Debug for WriteControl {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            WriteControl::Parser(p) => f.debug_tuple("Parser").field(&p).finish(),
            WriteControl::Response(r) => f.debug_tuple("Response").field(&tb(r)).finish(),
            WriteControl::StartTls => f.debug_tuple("StartTls").finish(),
            WriteControl::Shutdown => f.debug_tuple("Shutdown").finish(),
        }
    }
}
