...
use crate::smtp::{ReadControl, CodecControl};
...
pub struct SmtpCodec<IO> {
    ...
    sink: Sender<CodecControl>,
    recv: Receiver<CodecControl>,
}
...
impl<IO: Read + Write + MayBeTls> SmtpCodec<IO> {
    ...
    pub fn get_sender(&self) -> Sender<CodecControl> {
        self.sink.clone()
    }
            ...
            break match Pin::new(&mut projection.recv).poll_next(cx) {
                ...
                Poll::Pending => {
                    if let Some(pending) = projection.s2c_pending.pop_front() {
                        ...
                        match pending {
                            CodecControl::Shutdown => {
                                trace!("shutting down");
                                ready!(self.close_io(cx))?;
                                Poll::Ready(Ok(()))
                            }
                            CodecControl::StartData => {
                                trace!("starting data");
                                *projection.read_data = Some(true);
                                continue;
                            }
    ...
    pub fn send(&self, response: CodecControl) -> S2Fut<Result<()>> {
...
mod codec_tests {
    ...
    fn decode_finds_data_dot() -> Result<()> {
        ...
        assert_eq!(
            sender.start_send(CodecControl::StartData(SmtpReply::StartMailInputChallenge))?,
            ()
        );
