                ...
                WriteControl::Response(bytes) => {
                    match writer.poll_write(cx, bytes.as_ref()) {
                        ...
                        Poll::Ready(Ok(len)) => {
                            ...
                            s2c_pending.push_front(WriteControl::Response(bytes[len..].to_vec()));
                        }
                        Poll::Pending => {
                            s2c_pending.push_front(WriteControl::Response(bytes));
                            return Poll::Pending;
                        }
                        Poll::Ready(Err(e)) => {
                            s2c_pending.push_front(WriteControl::Response(bytes));
...
where
    IO: MayBeTls,
{
    pub fn send(&mut self, control: WriteControl) {
