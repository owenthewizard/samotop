    ...
    M: Sink<Bytes, Error = Error>,
{
    ...
    fn start_send(self: Pin<&mut Self>, mut item: ReadControl) -> Result<()> {
        match item {
            ReadControl::PeerConnected(ref mut conn) => {
                conn.extensions_mut().enable(SmtpExtension::PIPELINING);
                self.mail_service.extend(conn);
            }
            _ => {}
        }
