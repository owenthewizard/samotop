...
impl Response {
    ...
    /// Assert that the response code is within given set.
    /// If not, turn into a response error.
    /// [2] => matches all 2xx codes
    /// [35,2] => matches all 35x or 2xx codes
    /// [351] => matches exactly 351
    pub fn is<E: AsRef<[u16]>>(
        self,
        expected: E,
    ) -> std::result::Result<Response, crate::smtp::error::Error> {
        let code: u16 = self
            .code
            .to_string()
            .parse()
            .map_err(|_| Error::Client("Invalid response code"))?;
        for option in expected.as_ref() {
            let good = *option < 10 && *option == code / 100
                || *option < 100 && *option == code / 10
                || *option == code;
            if good {
                return Ok(self);
            }
...
mod test {
    ...
    fn test_response_from_str() {
        ...
        assert_eq!(
            raw_response.parse::<Response>().expect("should succeed"),
            Response {
                code: Code {
